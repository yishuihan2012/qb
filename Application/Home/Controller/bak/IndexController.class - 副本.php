...
class IndexController extends ConController {
    public function index(){
        ...
        $MP_goods=M("goodsmanage")->where("is_del=0")->field("id,g_price,g_title,g_salesnum,g_thumb,g_reco,g_type")->order("id desc")->select();
        if($MP_goods){
...
//                        $MP_data["u_level"]=3;
    ...
    public function ProductCon(){
        ...
        $MP_goods=M("goodsmanage")->where("is_del=0")->order("g_salesnum desc")->field("id,g_thumb,g_title,g_price")->limit(6)->select();
    ...
    public function SubmitOrder(){
        ...
        if($id){
            ...
            if($MP_gid){
                $MP_price = 0;
                $MP_goods[0] = M("goodsmanage")->where("id=" . $MP_gid)->field("id,g_title,g_vipprice,g_price,g_thumb,g_stock")->find();
...
//                $this->ginfo=$MP_goods[0]['ginfo'];
                ...
                foreach ($gid as $key => $val) {
                    ...
                    $MP_goods[$key] = M("goodsmanage")->where("id=" . $cart_info["c_gid"])->field("id,g_title,g_vipprice,g_price,g_thumb,g_stock")->find();

...
class ToolController extends Controller{ //工具类接口
  ...
  public function upLoadOriginalPic(){
        $upload = new \Think\Upload();// 实例化上传类
        ...
        if(!$info) {// 上传错误提示错误信息
            ...
            $img_path=HOST."Uploads/Uploads/certpicpath/".$info['img']['savepath'].$info['img']['savename'];
            echo json_encode(['code'=>200,'msg'=>'文件上传成功！','data'=>['url'=>$img_path]]);
        }
  }
  //获取手机验证码
  public function get_verification_code(){
    ...
    if(C('SMS_TYPE')==1){ //阿里云短信接口
        $mseeage=json_decode($this->sendSms($post['phone'],$code),true);
    }elseif(C('SMS_TYPE')==2){ //企业信使短信接口
        $mseeage=$this->CompanySms($post['phone'],$code);
    }
    // $mseeage['Code']='OK';
    if($mseeage['Code']!='OK')
     ...
     * 企业信使短信接口
     */
    public function CompanySms($phone,$code){
        $userid=C('CompanySmsUserid');
        $uname=C('CompanySmsUname');
        $pwd=C('CompanySmsPwd');
        $title=C('SYSTEM_TITLE');
        $content="【{$title}】您的短信验证码为{$code}";
        $url='http://sms1.ronglaids.com/sms.aspx?action=send&userid='.$userid.'&account='.$uname.'&password='.$pwd.'&mobile='.$phone.'&content='.$content.'&sendTime=&extno=';
...
//验证是否可以注册
  ...
  public function getSysConfig(){
       ...
       if($config){
            echo json_encode(['code'=>200,'msg'=>'信息获取成功！','data'=>['list'=>$config]]);
      }else{
           echo json_encode(['code'=>101,'msg'=>'信息获取失败！','data'=>'']);
       }
  }
